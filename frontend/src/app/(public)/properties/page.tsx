...
export default function PropertiesPage() {
  ...
  const openWhatsApp = (propertyTitle: string, phoneNumber: string) => {
    const defaultNumber = "212600000000"; // رقم افتراضي في حالة عدم وجود رقم للعقار
              ...
              <Card key={property.id} className="p-0 overflow-hidden group transition-shadow duration-300">
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex items-center text-gray-500">
                    ...
                    <span className="text-sm">{property.address}, {property.city.name}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {property.amenities.slice(0, 2).map((amenity) => (
                        <Badge key={amenity.name} className="bg-gray-100 text-gray-700">{amenity.name}</Badge>
                    ))}
                  ...
                  <div className="flex items-center space-x-2 rtl:space-x-reverse pt-4 border-t">
                    <button onClick={() => openWhatsApp(property.title, property.phone_number)} className="flex-1 flex items-center justify-center bg-blue-800 text-white px-4 py-2.5 rounded-lg hover:bg-blue-900 transition-colors font-semibold">
                        <WhatsAppIcon />
                        <span>طلب حجز</span>
                    </button>
